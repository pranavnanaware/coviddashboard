...
options = {
    "Cumulative Positive Results": 'positive',
...
with tab1:
    ...
    for chart in charts:
        ...
        df[options[chart]].loc[start_date: end_date + timedelta(days=1)].plot(
            label=chart, ax=ax)
        ax.set_xlabel('Date')
...
with tab3:
    ...
    st.markdown("**Results:** The scatter plot suggests that there is a diverse range of smoking rates among the adult population across different counties, which does not show a clear or consistent correlation with the number of COVID-19 cases reported in those counties. Further statistical analysis may be required to determine if there is a significant relationship between these variables, as this data is only for the top 15 counties.")
...
with tab4:
    st.subheader(
        'Predicting Regional Lockdown Based on Deaths and Cases per Population with Random Forest')
